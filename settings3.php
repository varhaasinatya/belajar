cek point logic juga apakah flexible
analisa fungsi backend growth pada code berikut :
settings.php
...
if (isset($_POST['reward_save']) && csrf_verify($_POST['csrf_token'])) {
    ...
    if (prevent_form_resubmission('reward_save', 'settings.php', ['komisi_produk' => $_POST['reward_product_id'] ?? 0, 'success' => 1])) {
        ...
        // Validasi khusus point_growth dan hybrid & Point logic 
			...
			$point_logic = $_POST['point_logic'] ?? 'akumulasi';
		}
...
<div class="container py-4">
    ...
    <div class="bg-setting shadow mb-4">
        ...
        <form method="post" class="row g-3 mb-4" id="rewardForm">
			...
			<!-- LOGIKA PEROLEHAN POINT -->
			<div class="col-md-3 reward-field reward-growth" style="display:none;">
				<label>Logika Perolehan Point *</label>
				<div>
					<input type="radio" name="point_logic" value="akumulasi" id="point_logic_akumulasi"
						<?= (!isset($reward_edit['point_logic']) || $reward_edit['point_logic']=='akumulasi')?'checked':'' ?>>
					<label for="point_logic_akumulasi">
						Akumulasi
						<span title="Point dihitung dari total semua kaki (misal kiri+kanan)">
							<i class="fas fa-info-circle"></i>
						</span>
					</label>
					&nbsp;
					<input type="radio" name="point_logic" value="terkuat" id="point_logic_terkuat"
						<?= (isset($reward_edit['point_logic']) && $reward_edit['point_logic']=='terkuat')?'checked':'' ?>>
					<label for="point_logic_terkuat">
						Kaki Terkuat
						<span title="Point hanya dihitung dari kaki yang paling banyak tim/membernya">
							<i class="fas fa-info-circle"></i>
						</span>
					</label>
				</div>
				<div class="form-text">
					Akumulasi = point dari total semua kaki.<br>
					Kaki Terkuat = point hanya dari kaki yang timnya terbanyak.
				</div>
			</div>
