...
if (isset($_POST['level_reward_save']) && csrf_verify($_POST['csrf_token'])) {
    // Terapkan pencegahan resubmit
    if (prevent_form_resubmission('level_reward_save', 'admin_panel_settings.php', ['komisi_produk' => $_POST['reward_product_id'] ?? 0, 'success' => 1])) {
        ...
        // Ambil network type dari database
        $network_type = get_network_type($conn);
        
        // Siapkan data reward
        $reward_data = [
            'id' => $reward_id > 0 ? $reward_id : null,
        ...
        // Simpan reward level
        $result = save_full_level_reward($conn, $reward_data);
        
        if ($result['success']) {
            header("Location: admin_panel_settings.php?komisi_produk=$product_id&success=1&msg=" . urlencode($result['message']));
            exit;
        } else {
...
if (isset($_GET['delete_level_reward']) && intval($_GET['delete_level_reward']) > 0) {
    ...
    if ($product_id > 0) {
        header("Location: admin_panel_settings.php?komisi_produk=$product_id&success=1&msg=" . urlencode("Reward level berhasil dihapus"));
        exit;
    } else {
...
<div class="container py-4">
    ...
    <div class="bg-setting shadow mb-4">
		...
		<?php if ($komisi_edit): // Pastikan dalam konteks edit produk ?>
		<div class="bg-setting shadow mb-4">
			...
			<form method="post" class="row g-3">
				...
				<div class="col-12">
					...
					<?php if ($level_reward_edit): ?>
						<a href="admin_panel_settings.php?komisi_produk=<?= $komisi_edit['id'] ?>" class="btn btn-success">
					...
					<tbody>
						<?php if($level_rewards): $no=1; foreach($level_rewards as $reward): ?>
							<tr>
								...
								<td>
									<a href="admin_panel_settings.php?komisi_produk=<?= $komisi_edit['id'] ?>&edit_level_reward=<?= $reward['id'] ?>" 
									   class="btn btn-sm btn-warning btn-action">
										Edit
									</a>
									<a href="admin_panel_settings.php?komisi_produk=<?= $komisi_edit['id'] ?>&delete_level_reward=<?= $reward['id'] ?>" 
                    ...
                    <div class="row mb-2">
                        <div class="col-md-6">
                            <div class="input-group">
                                ...
                                <input type="number" min="1" class="form-control" name="qualification[]" value="<?= ($reward_edit['reward_type']??'')=='referral' ? htmlspecialchars($reward_edit['point'] ?? 10) : '10' ?>" required>
                        ...
                        <div class="col-md-6">
                            <div class="input-group">
                                ...
                                <input type="number" min="0" class="form-control" name="qualification_reward[]" value="<?= ($reward_edit['reward_type']??'')=='referral' ? htmlspecialchars($reward_edit['reward_amount'] ?? 0) : '' ?>">
                    ...
                    if ($rw['reward_type'] == 'referral') {
                        ...
                        foreach ($grouped_ids as $desc => $ids) {
                            if (in_array($rw['id'], $ids)) {
                                ...
                                echo '<a href="settings.php?komisi_produk=' . $komisi_edit['id'] . '&hapus_reward=' . $first_id . '&hapus_related=1" class="btn btn-sm btn-danger btn-action" onclick="return confirm(\'Delete this reward and all related qualifications?\')">Delete</a>';
                                echo '</td>';
                                echo '</tr>';
